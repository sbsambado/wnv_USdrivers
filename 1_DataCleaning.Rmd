...
##Purpose
Format data to make it easier to merge into one dataset for GAM analysis where GEOID is unique row and columns are covariate data
...
## i decided to upload data files within the sections 

```

##Spatial boundaries
```{r}
## set spatial boundaries to standardized spatial extraction
...
```{r}
## CT is weird because they changed their GEOIDs to new planning regions so I have to revert new codes to old ones so they merge with full dataset
...
##Landscape metrics
(this code takes the longest to run of them all - it probably could be streamlined or ran on parrells)
Figure 1a
```{r}
## A. upload data
...
# landscape metrics https://r-spatialecology.github.io/landscapemetrics/ (i found this helpful)
## A. upload data
...
# high value = More fragmented or interspersed land use — urban and agricultural areas are closely mixed.
...
## this is the bulk of data for GAM, need to add R0(T) from next calculation
